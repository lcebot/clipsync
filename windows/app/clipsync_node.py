...
A persisted id would buy one thing, a restart being recognised as the same node, but at the cost of
atomic writes, a half-written-id failure mode, an installer that has to know not to touch it, and,
worst, a value someone could hand-edit into a collision. Two nodes sharing one id breaks dedup and
relay selection in ways that are very hard to trace back to their cause, and a file on disk is an
invitation to exactly that. A fresh UUID per start cannot collide at all.
...
Log continuity is handled separately: each side logs its own id in its first line, so every later
`client <id> connected` has a session to belong to.
"""
import socket
import uuid
...
def declaration(cfg=None) -> dict:
    ...
    node out of everyone *else's* election, before anyone asks. The second layer, which is refusing a
    ...
    two nodes must open them: both opening means the file is transferred twice, while neither opening
    means it is not transferred at all.  This PC opens them as well as accepting them, so it says true, and
    the rule is the symmetric one: **the node that dialled opens them**, which both ends work out
    from the same two facts (see SecureChannel.drives_transfer).  Getting this field wrong between
    two PCs, with both saying false, is not merely inefficient: neither end would open anything, so
    every WANT would go unanswered and the transfer would simply hang.
