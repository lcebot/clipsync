 ...
 *
 * <p>Each concern (gating, connecting, the handshake, the screen-off burst, the heartbeat, the read
 ...
 *
 * <ul>
 *   <li><b>the heartbeat.</b> One timer for the device, not one per link, because N pingers would wake
 ...
 * </ul>
 *
 * <p>{@link #sentHash} is per link rather than shared across them: a clip has to reach every peer,
...
final class Link implements AutoCloseable {
     ...
     *
     * <p>Per link, not global: a clip has to reach every peer, including one that connects a minute
     ...
     *
     * <p>A single state, because the three possibilities are mutually exclusive: a link is running,
     ...
     *
     * <ul>
     *   <li>{@link #RUNNING} also covers a link that ended by <em>failing</em>, such as a read that
     ...
     * How this link ended.
     *
     * <p>Only ever asked as {@link #endedOnPurpose()}; nothing needs to tell PEER_BYE from
     ...
     *
     * <p>The single question the dialler's back-off logic needs: whichever end decided, redialling
     ...
     * offered file or anything newer, then let the link drop so the radio can sleep. While an offer is open
