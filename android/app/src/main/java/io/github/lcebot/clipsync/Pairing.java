 ...
 * because the PSK is what is being transferred, so <b>a nine-digit code is the whole of the
 * authentication</b>: shown on the device that holds the key, typed on the device that wants it.
 *
 * <p><b>The code is nine digits, and nothing else.</b> It is <em>shown</em> in groups of three, as
 ...
 *
 * <h2>Why the code is stretched, not just hashed</h2>
 *
 * <em>Online</em> guessing is bounded properly, since five failed handshakes end the window, but a
 ...
 *
 * <p>scrypt, N=2¹⁴, r=8, p=10. Hashcat mode 8900 measures scrypt at exactly N=16384, r=8, p=1 and
 ...
 * buffer, so p=10 divides that by 10: ≈713 H/s. 10⁹ ÷ 713 ≈ <b>16 days</b> to exhaust one RTX 4090
 * walking the whole code space, ~8 days to expect a hit.
 *
 * <p>For scale: a plain PBKDF2-HMAC-SHA256 at 600 000 rounds, priced on the same hardware and the
 * same code space, would fall in <b>~21 hours</b>; hashcat mode 27500 measures PBKDF2-HMAC-SHA256
 ...
 * roughly <b>50 000×</b> harder to exhaust than that baseline. The multiplier is not the interesting
 * part; <em>where</em> it comes from is. PBKDF2's state is a few dozen bytes, so a GPU runs as many
 ...
 * mode 9300 is scrypt at the same N=16384 with r=1 (2 MiB) and runs at 83 890 H/s, so <b>8× the memory
 * costs the GPU 11.8×</b>, superlinearly, which is the memory-hardness doing something PBKDF2
 ...
 * What this does achieve is making recording a pairing <em>on the off-chance</em> not worth the
...
final class Pairing {
     ...
     * with {@link Crypto#fromHex}, which answers null rather than throwing, because a truncated or foreign
     ...
     * are taken back out.
     *
     * <p>A ninth glyph is a real cost to a user reading a code across a room, but the answer to that
     * is not a shorter code, it is a better-shaped one: nine digits are <em>shown</em> as three
     * groups of three ({@link #grouped}), which keeps the code as easy to read aloud as a shorter
     ...
     * <h3>Why scrypt</h3>
     *
     * <b>Conscrypt does not implement PBKDF2 at all</b>: {@code SecretKeyFactory.getInstance(
     * "PBKDF2WithHmacSHA256")} resolves to the BouncyCastle copy Android bundles
     * ({@code com.android.org.bouncycastle}), pure Java with no native or hardware acceleration.
     * Measured, 310 000 rounds of {@code PBKDF2WithHmacSha256} take roughly 2.2 s on a Pixel 3
     * (Daniel Hugenroth, "Password hashing on Android"), so 600 000 rounds of PBKDF2 would cost
     * <b>about 4 s on a Pixel 3 and 1.5 to 2.5 s on a newer mid-range phone</b>, several seconds of a
     ...
     *       milliseconds of native code; p=10 makes it ten of those, so roughly <b>0.5 to 1.0 s</b>.
     ...
     *
     * <p>Under a second is short, but not free: {@link #channelKey} runs on a worker thread and the
    ...
    /**
     * scrypt's parallelisation parameter p, which is the whole of the time budget; see {@link #SCRYPT_N}.
     *
     * <p>Ten, and the ceiling is a guess rather than a measurement. <b>Nobody has ever timed this on
     * a real phone.</b> The 0.5 to 1.0 s quoted above is an extrapolation from scrypt's own paper
     ...
     * to <b>p=8</b>. Below 8 is not worth doing: at {@link #CODE_DIGITS} digits, p=8 is already
     ...
     * avoids {@code Math.abs(nextInt())}, which is negative for {@code Integer.MIN_VALUE}, the one
     * input abs has no answer for.
     *
     * <p>{@code 1_000_000_000} still fits an {@code int}, because {@link Integer#MAX_VALUE} is
     * 2 147 483 647, so the bound is exact and {@code nextInt} stays unbiased over every one of the
     * 10⁹ codes. A tenth digit would not fit and would need {@code nextLong(bound)} or two draws.
     *
     * <p>Formatted with leading zeros, because "007421234" is nine digits and 7421234 is not, so a
    ...
    /**
     * The code as a person should see it: the code {@code "123456789"} is displayed as
     * {@code "123 456 789"}.
     *
    ...
    /**
     * Everything that is not a digit, removed; this is the inverse of {@link #grouped}, and the gate the
